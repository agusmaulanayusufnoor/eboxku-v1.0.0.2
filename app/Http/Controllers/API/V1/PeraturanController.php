...
class PeraturanController extends BaseController
{
     ...
     * @return void
     */
    public function __construct(Peraturan $peraturan)
    {
    ...
    public function index()
    {
        ...
        //$peraturan= $this->peraturan->latest()->get();
        if($levelLogin === 'admin'){
            $peraturan  = DB::table('peraturan')
            ->join('kode_kantors', 'peraturan.kantor_id', '=', 'kode_kantors.id')
            ->select('peraturan.id','peraturan.namafile','peraturan.tanggal','peraturan.file',
            'peraturan.kantor_id','kode_kantors.nama_kantor')
            ->orderBy('id','desc')
            ->get();
        }else{
            $peraturan  = DB::table('peraturan')
        ...
        return $this->sendResponse($peraturan, 'peraturan list');
    }
    ...
    public function destroy($id)
    {
        ...
        $peraturan = $this->peraturan->findOrFail($id);

        $peraturan->delete();
        $file = public_path("file/peraturan/".$peraturan->file);
        if (! file_exists($file)){
            //session()->flash('hapus','file sudah dihapus');
            return $this->sendResponse($peraturan, 'File sudah dihapus!');
        }else{
            unlink("file/peraturan/".$peraturan->file);
