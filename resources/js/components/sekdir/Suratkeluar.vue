...
//   },
    ...
    async cekNorek (){
      if(this.$gate.isAdmin() || this.$gate.isSekdir() ){
        ...
        //const response = await axios.get('api/suratkeluar/ceknama')
          const response = await axios.post('api/suratkeluar/ceknorek',formData)
          //this.cekNorekData = response.data.data[0].no_surat;
          if (response.data.message=='adarek'){
            this.cekNorekData = response.data.data[0].no_surat;
            this.pesaneror = 'No suratkeluar '+this.cekNorekData+' Sudah Ada'
         ...
         this.$Progress.start();
            ...
            if(this.$gate.isAdmin() || this.$gate.isSekdir() ){
             ...
             axios.get("api/suratkeluar")
                .then((response) => {
                this.suratkeluar = response.data.data;
     ...
     createUser(){
         ...
         this.$Progress.start();
           ...
           // console.log(this.file);
            axios.post('api/suratkeluar',formData,config)
              .then((response)=>{
                  $('#addNew').modal('hide');
              ...
              .catch(error =>{
                  ...
                  Toast.fire({
                      icon: 'error',
                      title: error.response.data.errors.no_surat[0]
          ...
          updateUser(){
                ...
                this.form.put('api/suratkeluar/'+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                ...
                Swal.fire({
                    ...
                    confirmButtonText: 'Ya, Hapus!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('api/suratkeluar/'+id).then(()=>{
                                        Swal.fire(
                                        'Dihapus!',
                                        'Data telah dihapus.',
                                        'success'
