        ...
        <v-col cols="11">
          <v-card class="pa-2 mx-auto" v-if="$gate.isAdmin() || $gate.isAK()">
            <v-toolbar
              ...
              shaped
            >
              <v-toolbar-title> File Kas Kecil </v-toolbar-title>
              <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    getOtorisator() {
      if (this.$gate.isAdmin() || this.$gate.isAK()) {
        ...
        axios
          .get("api/kaskecil/getotorisator")
          .then((response) => {
    ...
    async filterKantor() {
      ...
      if (this.nama_kantor != "") {
        if (this.$gate.isAdmin()) {
          axios
            .get("api/kaskecil/filterkantor", {
              params: {
                kantor_id: this.nama_kantor,
              },
            })
            .then((response) => {
              this.kaskecil = response.data.data;
  ...
  `,
    ...
    async filterOtorisator() {
      ...
      if (this.otorisator_id != "") {
        if (this.$gate.isAdmin() || this.$gate.isAK()) {
          axios
            .get("api/kaskecil/filterotorisator", {
              params: {
  ...
  `,
    ...
    initialize() {
      ...
      if (this.$gate.isAdmin() || this.$gate.isAK()) {
        //axios.get("api/user").then((response) => {(this.users = response.data.data)});
        axios.get("api/kaskecil").then((response) => {
          this.kaskecil = response.data.data;
          this.kantor_id = this.$kantor_id;
          // this.form.fill
          //console.log(this.kaskecil);
    ...
    createUser() {
      ...
      axios
        .post("api/kaskecil", formData, config)
        .then((response) => {
          $("#addNew").modal("hide");
