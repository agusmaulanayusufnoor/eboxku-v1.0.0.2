        ...
        <v-col cols="11">
          <v-card class="pa-2 mx-auto" v-if="$gate.isAdmin() || $gate.isAK()">
            <v-toolbar
              ...
              shaped
            >
              <v-toolbar-title> File Rekening Koran ABA </v-toolbar-title>
              <v-spacer></v-spacer>
                    ...
                    <v-container fluid>
                      ...
                      >
                        <template v-slot:label>
                          <div>
                            <strong class="text-h6 text-bold"
                              >Jenis Simpanan :</strong
                            >
                          </div>
                        </template>
                        <v-radio label="Tabungan" value="tabungan"></v-radio>
                        <v-radio label="Deposito" value="deposito"></v-radio>
...
export default {
  data: (vm) => ({
    ...
    dateFormatted: "",
    tanggal: "",
    tanggalRules: [(v) => !!v || "Periode bulan belum diisi"],
  ...
  created() {
    ...
    if (this.$refs.cbjenis) {
      this.$refs.cbjenis.reset();
      this.jenis = null; // Reset model v-model juga
    }
  },
  ...
  methods: {
    ...
    async filterKantor() {
      ...
      if (this.nama_kantor != "") {
        if (this.$gate.isAdmin()) {
          axios
            .get("api/rekkoranaba/filterkantor", {
              params: {
    ...
    async filterJenis() {
      ...
      if (this.jenis != "") {
        if (this.$gate.isAdmin() || this.$gate.isAK()) {
          axios
            .get("api/rekkoranaba/filterjenis", {
              params: {
                jenis: this.jenis,
              },
            })
    ...
    initialize() {
      ...
      if (this.$gate.isAdmin() || this.$gate.isAK()) {
        //axios.get("api/user").then((response) => {(this.users = response.data.data)});
        axios.get("api/rekkoranaba").then((response) => {
          this.rekkoranaba = response.data.data;
          this.kantor_id = this.$kantor_id;
          // this.form.fill
          // console.log(this.rekkoranaba);
    ...
    createUser() {
      ...
      axios
        .post("api/rekkoranaba", formData, config)
        .then((response) => {
          $("#addNew").modal("hide");
    ...
    updateUser() {
      ...
      this.form
        .put("api/rekkoranaba/" + this.form.id)
        .then((response) => {
          // success
          $("#addNew").modal("hide");
          Toast.fire({
