        ...
        <v-col cols="11">
          <v-card class="pa-2 mx-auto" v-if="$gate.isAdmin() || $gate.isAK()">
            <v-toolbar
              ...
              shaped
            >
              <v-toolbar-title> File Overbooking </v-toolbar-title>
              <v-spacer></v-spacer>
  ...
  methods: {
    ...
    getOtorisator() {
      if (this.$gate.isAdmin() || this.$gate.isAK()) {
        ...
        axios
          .get("api/overbooking/getotorisator")
          .then((response) => {
    ...
    async filterKantor() {
      ...
      if (this.nama_kantor != "") {
        if (this.$gate.isAdmin()) {
          axios
            .get("api/overbooking/filterkantor", {
              params: {
    ...
    async filterOtorisator() {
      ...
      if (this.otorisator_id != "") {
        if (this.$gate.isAdmin() || this.$gate.isAK()) {
          axios
            .get("api/overbooking/filterotorisator", {
              params: {
    ...
    initialize() {
      ...
      if (this.$gate.isAdmin() || this.$gate.isAK()) {
        //axios.get("api/user").then((response) => {(this.users = response.data.data)});
        axios.get("api/overbooking").then((response) => {
          this.overbooking = response.data.data;
          this.kantor_id = this.$kantor_id;
          // this.form.fill
          //console.log(this.overbooking);
    ...
    createUser() {
      ...
      console.log(this.kantor_id);
      axios
        .post("api/overbooking", formData, config)
        .then((response) => {
          $("#addNew").modal("hide");
    ...
    updateUser() {
      ...
      this.form
        .put("api/overbooking/" + this.form.id)
        .then((response) => {
          // success
          $("#addNew").modal("hide");
          Toast.fire({
