        ...
        <v-col cols="11">
          <v-card class="pa-2 mx-auto" v-if="$gate.isAdmin() || $gate.isAK()">
            <v-toolbar src="images/banner-red.jpg" dark shaped>
              <v-toolbar-title> File Memo Overbooking </v-toolbar-title>
              <v-spacer></v-spacer>
...
export default {
  data: (vm) => ({
    ...
    tanggal: new Date(Date.now() - new Date().getTimezoneOffset() * 60000)
      .toISOString()
      .substr(0, 10),
    tanggalRules: [(v) => !!v || "Tanggal memoob belum diisi"],
  ...
  methods: {
    ...
    initialize() {
      ...
      if (this.$gate.isAdmin() || this.$gate.isAK()) {
        axios.get("api/memoob").then((response) => {
          this.memoob = response.data.data;
          this.kantor_id = this.$kantor_id;
          // this.form.fill
    ...
    createUser() {
      ...
      axios
        .post("api/memoob", formData, config)
        .then((response) => {
          $("#addNew").modal("hide");
