    ...
    async cekNorek (){
      if(this.$gate.isAdmin() || this.$gate.isUM() ){
        ...
        //const response = await axios.get('api/akta/ceknama')
          const response = await axios.post('api/akta/ceknorek',formData)
          //this.cekNorekData = response.data.data[0].no_akta;
          if (response.data.message=='adarek'){
            this.cekNorekData = response.data.data[0].no_akta;
            this.pesaneror = 'No AKTA '+this.cekNorekData+' Sudah Ada'
     ...
     createUser(){
         ...
         this.$Progress.start();
           ...
           // console.log(this.file);
            axios.post('api/akta',formData,config)
              .then((response)=>{
                  $('#addNew').modal('hide');
              ...
              .catch(error =>{
                  ...
                  Toast.fire({
                      icon: 'error',
                      title: error.response.data.errors.no_akta[0]
          ...
          updateUser(){
                ...
                this.form.put('api/akta/'+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                ...
                Swal.fire({
                    ...
                    confirmButtonText: 'Ya, Hapus!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('api/akta/'+id).then(()=>{
                                        Swal.fire(
                                        'Dihapus!',
                                        'Data telah dihapus.',
                                        'success'
