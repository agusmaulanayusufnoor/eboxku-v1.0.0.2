                        ...
                        <div class="form-group input-group">
                            ...
                            <has-error :form="form" field="namafile"></has-error>
                             <v-text-field
                                ...
                                name="namafile"
                                label="Nama File"
                                placeholder="Nama File: 'se_namasurat'"
                                outlined
                                required
                                dense
                                prepend-icon="mdi-file"
...
//   },
    ...
    async cekNorek (){
      if(this.$gate.isAdmin() || this.$gate.isUM() ){
        ...
        //const response = await axios.get('api/sedir/ceknama')
          const response = await axios.post('api/sedir/ceknorek',formData)
          //this.cekNorekData = response.data.data[0].no_se;
          if (response.data.message=='adarek'){
            this.cekNorekData = response.data.data[0].no_se;
            this.pesaneror = 'No SE '+this.cekNorekData+' Sudah Ada'
     ...
     createUser(){
         ...
         this.$Progress.start();
           ...
           // console.log(this.file);
            axios.post('api/sedir',formData,config)
              .then((response)=>{
                  $('#addNew').modal('hide');
              ...
              .catch(error =>{
                  ...
                  Toast.fire({
                      icon: 'error',
                      title: error.response.data.errors.no_se[0]
          ...
          updateUser(){
                ...
                this.form.put('api/sedir/'+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                ...
                Swal.fire({
                    ...
                    confirmButtonText: 'Ya, Hapus!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('api/sedir/'+id).then(()=>{
                                        Swal.fire(
                                        'Dihapus!',
                                        'Data telah dihapus.',
                                        'success'
