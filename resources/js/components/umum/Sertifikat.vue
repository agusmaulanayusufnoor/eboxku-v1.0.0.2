    ...
    computed: {
        headers(){
            let headers = [
                ...
                { text: 'No Sertifikat', value: 'no_sertifikat' },
...
//   },
    ...
    async cekNorek (){
      if(this.$gate.isAdmin() || this.$gate.isUM() ){
        ...
        //const response = await axios.get('api/sertifikat/ceknama')
          const response = await axios.post('api/sertifikat/ceknorek',formData)
          //this.cekNorekData = response.data.data[0].no_sertifikat;
          if (response.data.message=='adarek'){
            this.cekNorekData = response.data.data[0].no_sertifikat;
            this.pesaneror = 'No Sertifikat '+this.cekNorekData+' Sudah Ada'
      ...
      initialize() {
         this.$Progress.start();
            ...
            if(this.$gate.isAdmin() || this.$gate.isUM() ){
             ...
             axios.get("api/sertifikat")
                .then((response) => {
                this.sertifikat = response.data.data;
     ...
     createUser(){
         ...
         this.$Progress.start();
           ...
           // console.log(this.file);
            axios.post('api/sertifikat',formData,config)
              .then((response)=>{
                  $('#addNew').modal('hide');
              ...
              .catch(error =>{
                  ...
                  Toast.fire({
                      icon: 'error',
                      title: error.response.data.errors.no_sertifikat[0]
          ...
          updateUser(){
                ...
                this.form.put('api/sertifikat/'+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                ...
                Swal.fire({
                    ...
                    confirmButtonText: 'Ya, Hapus!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('api/sertifikat/'+id).then(()=>{
                                        Swal.fire(
                                        'Dihapus!',
                                        'Data telah dihapus.',
                                        'success'
