    ...
    <v-container fluid>
      <v-row no-gutters class="justify-content-md-center">
        <v-col cols="11">
          <v-card
            class="pa-2 mx-auto"
            v-if="$gate.isAdmin() || $gate.isUM() || $gate.isSekdir()"
          >
            <v-toolbar
              ...
              shaped
            >
              <v-toolbar-title> File Memo Umum </v-toolbar-title>
              <v-spacer></v-spacer>
                  ...
                  <template>
                    <v-container fluid>
                      ...
                      >
                        ...
                        <v-radio label="CSR" value="csr"></v-radio>
                      </v-radio-group>
                    </v-container>
                  </template>
  ...
  methods: {
    ...
    initialize() {
      ...
      if (this.$gate.isAdmin() || this.$gate.isUM() || this.$gate.isSekdir()) {
        //axios.get("api/user").then((response) => {(this.users = response.data.data)});
        axios.get("api/memoumum").then((response) => {
          this.memoumum = response.data.data;
          this.kantor_id = this.$kantor_id;
          // this.form.fill
          // console.log(this.memoumum);
    ...
    createUser() {
      ...
      axios
        .post("api/memoumum", formData, config)
        .then((response) => {
          $("#addNew").modal("hide");
