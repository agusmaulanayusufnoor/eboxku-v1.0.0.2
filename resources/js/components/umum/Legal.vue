    ...
    computed: {
        headers(){
            let headers = [
                ...
                { text: 'No Legalitas', value: 'no_legal' },
...
//   },
    ...
    async cekNorek (){
      if(this.$gate.isAdmin() || this.$gate.isUM() ){
        ...
        //const response = await axios.get('api/legal/ceknama')
          const response = await axios.post('api/legal/ceknorek',formData)
          //this.cekNorekData = response.data.data[0].no_legal;
          if (response.data.message=='adarek'){
            this.cekNorekData = response.data.data[0].no_legal;
            this.pesaneror = 'No Legalitas '+this.cekNorekData+' Sudah Ada'
     ...
     createUser(){
         ...
         this.$Progress.start();
           ...
           // console.log(this.file);
            axios.post('api/legal',formData,config)
              .then((response)=>{
                  $('#addNew').modal('hide');
              ...
              .catch(error =>{
                  ...
                  Toast.fire({
                      icon: 'error',
                      title: error.response.data.errors.no_legal[0]
          ...
          updateUser(){
                ...
                this.form.put('api/legal/'+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                ...
                Swal.fire({
                    ...
                    confirmButtonText: 'Ya, Hapus!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('api/legal/'+id).then(()=>{
                                        Swal.fire(
                                        'Dihapus!',
                                        'Data telah dihapus.',
                                        'success'
