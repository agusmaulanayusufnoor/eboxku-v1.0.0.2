...
<v-app>
    <v-container fluid>
        <v-row no-gutters class="justify-content-md-center">
          <v-col cols="11">
            <v-card class="pa-2 mx-auto" v-if="$gate.isAdmin() || $gate.isSkai()">
              <v-toolbar src="images/banner-orange.jpg" color="yellow" dark shaped>
                <v-toolbar-title>
        ...
        <div v-if="!$gate.isAdmin() && !$gate.isSkai()">
            <not-found></not-found>
        </div>
      ...
      initialize() {
         this.$Progress.start();
            ...
            if(this.$gate.isAdmin() || this.$gate.isSkai() ){
             ...
             axios.get("api/periksa")
                .then((response) => {
                this.periksa = response.data.data;
     ...
     createUser(){
         ...
         this.$Progress.start();
           ...
           // console.log(this.file);
            axios.post('api/periksa',formData,config)
              .then((response)=>{
                  $('#addNew').modal('hide');
          ...
          updateUser(){
                ...
                this.form.put('api/periksa/'+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                ...
                Swal.fire({
                    ...
                    confirmButtonText: 'Ya, Hapus!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('api/periksa/'+id).then(()=>{
                                        Swal.fire(
                                        'Dihapus!',
                                        'Data telah dihapus.',
                                        'success'
