    ...
    <v-container fluid>
      <v-row no-gutters class="justify-content-md-center">
        <v-col cols="11">
          <v-card
            class="pa-2 mx-auto"
            v-if="$gate.isAdmin() || $gate.isPelayanan() || $gate.isBisnis() || $gate.isCs() || $gate.isTeller()"
          >
            <v-toolbar src="images/banner-biru-pelayanan.jpg" dark shaped>
              ...
              <v-spacer></v-spacer>
              <v-btn small color="indigo" dark @click="newModal" v-if="$gate.isAdmin() || $gate.isPelayanan() || $gate.isCs() || $gate.isTeller()">
      ...
      <div v-if="!$gate.isAdmin() && !$gate.isPelayanan() && !$gate.isBisnis() && !$gate.isCs() && !$gate.isTeller()">
        <not-found></not-found>
      </div>
...
export default {
  ...
  methods: {
    ...
    initialize() {
      ...
      if (this.$gate.isAdmin() || this.$gate.isPelayanan() || this.$gate.isBisnis() || this.$gate.isCs() || this.$gate.isTeller()) {
        const params = {};
        ...
        axios.get("api/permoperasional", { params }).then((response) => {
