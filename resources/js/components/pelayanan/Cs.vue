...
//   },
    ...
    methods: {
        async cekTgl (){
            if(this.$gate.isAdmin() || this.$gate.isPelayanan() ){
                ...
                const response = await axios.post('api/cs/cektgl',formData)
    ...
    initialize() {
      ...
      if(this.$gate.isAdmin() || this.$gate.isPelayanan() || this.$gate.isCs()){
        ...
        axios.get("api/cs", { params }).then((response) => {
          this.cs = response.data.data;
     ...
     createUser(){
         ...
         this.$Progress.start();
           ...
           // console.log(this.file);
            axios.post('api/cs',formData,config)
              .then((response)=>{
                  $('#addNew').modal('hide');
          ...
          updateUser(){
                ...
                this.form.put('api/cs/'+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                ...
                Swal.fire({
                    ...
                    confirmButtonText: 'Ya, Hapus!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('api/cs/'+id).then(()=>{
                                        Swal.fire(
                                        'Dihapus!',
                                        'Data telah dihapus.',
                                        'success'
