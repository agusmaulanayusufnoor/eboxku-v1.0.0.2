      ...
      <v-row no-gutters class="justify-content-md-center">
        <v-col cols="11">
          ...
          >
            ...
            >
              <v-toolbar-title>
                <v-icon left>mdi-file-invoice</v-icon> Surat Perintah Pengeluaran Uang
              </v-toolbar-title>
              <v-spacer></v-spacer>
              <v-btn
                v-if="$gate.isAdmin() || $gate.isPelayanan() || $gate.isTeller()"
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" v-show="!editmode">Tambah SPPU</h5>
              <h5 class="modal-title" v-show="editmode">Edit SPPU</h5>
              <button
                type="button"
                class="close"
                data-dismiss="modal"
            ...
            <v-form
              @submit.prevent="editmode ? updateSppu() : createSppu()"
              ref="form"
              v-model="valid"
              lazy-validation
            >
              <div class="modal-body">
                <div class="form-group">
                  <v-col cols="12" sm="12" md="12">
                    <v-text-field
                      v-model="editedItem.penerima_uang"
                      label="Penerima Uang"
                      outlined
                      required
                      dense
                      ...
                      counter="150"
                      :rules="[(v) => !!v || 'Penerima uang harus diisi', (v) => (v && v.length <= 150) || 'Maksimal 150 karakter']"
                    ></v-text-field>
  ...
  computed: {
    headers() {
      let headers = [
        ...
        { text: "Penerima Uang", value: "penerima_uang" },
  ...
  methods: {
    ...
    initialize() {
      ...
      if (this.$gate.isAdmin() || this.$gate.isPelayanan() || this.$gate.isTeller()) {
        axios.get("api/sppu").then((response) => {
          this.sppuList = response.data.data;
        });
      }
      this.$Progress.finish();
    },
    ...
    applyFilter() {
      ...
      if (this.filterPenerima) {
        params.penerima_uang = this.filterPenerima;
      }
    ...
    newModal() {
      this.editmode = false;
      this.editedItem = { id: "", penerima_uang: "", nominal: 0, keterangan: "" };
      $("#addNew").modal("show");
    },
    ...
    updateSppu() {
      this.$Progress.start();
      axios
        .put("api/sppu/" + this.editedItem.id, this.editedItem)
        .then((response) => {
          $("#addNew").modal("hide");
          Toast.fire({
            icon: "success",
