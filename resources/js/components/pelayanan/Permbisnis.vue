        ...
        <v-col cols="11">
          ...
          >
            <v-toolbar src="images/banner-biru-pelayanan.jpg" dark shaped>
              <v-toolbar-title> Permohonan ke Divisi Bisnis </v-toolbar-title>
              <v-spacer></v-spacer>
            ...
            <div class="card-body table-responsive p-0">
              ...
              >
                ...
                <template v-slot:item.file_memo="{ item }">
                  <v-card-actions class="justify-center">
                    <v-icon
                      small
                      color="blue"
                      class="mr-4"
                      @click="
                        downloadFileMemo(item.id, item.file_memo)
                      "
                    >
                      mdi-download
                    </v-icon>
  ...
  methods: {
    ...
    initialize() {
      ...
      if (this.$gate.isAdmin() || this.$gate.isPelayanan() || this.$gate.isBisnis()) {
        ...
        axios.get("api/permbisnis", { params }).then((response) => {
          this.permbisnis = response.data.data;
    ...
    createUser() {
      ...
      axios
        .post("api/permbisnis", formData, config)
        .then((response) => {
          $("#addNew").modal("hide");
    ...
    downloadFile(id, file) {
      axios({
        url: "api/permbisnis/download/" + id,
        method: "GET",
        responseType: "blob",
      })
        .then((response) => {
    ...
    downloadFileMemo(id, file_memo) {
      axios({
        url: "api/permbisnis/filememo/" + id,
        method: "GET",
        responseType: "blob",
      })
        .then((response) => {
    ...
    updateUser() {
      ...
      if (this.$gate.isBisnis()){
        formData.set("tgl_setujutolak", this.tgl_permohonan);
        formData.set("file_memo", this.file);
      }
      if (this.$gate.isAdmin()) {
        formData.set("tgl_acc", this.tgl_permohonan);
        formData.set("tgl_selesai", this.tgl_permohonan);
      ...
      axios
        .post("api/permbisnis/" + this.editedItem.id, formData, config)
