...
<v-app>
    <v-container fluid>
        <v-row no-gutters class="justify-content-md-center">
          <v-col cols="11">
            <v-card class="pa-2 mx-auto" v-if="$gate.isAdmin() || $gate.isKredit() || $gate.isBisnis() || $gate.isPelayanan() || $gate.isCs()">
        ...
        <div v-if="!$gate.isAdmin() && !$gate.isKredit() && !$gate.isBisnis() && !$gate.isPelayanan() && !$gate.isCs()">
            <not-found></not-found>
        </div>
                            ...
                            >
                                ...
                                <template v-slot:activator="{ on, attrs }">
                                    <v-text-field
                                        ...
                                        :rules="tanggalRules"
                                        label="Tanggal Pelunasan Kredit"
    ...
    data: vm => ({
     ...
     valid:true,
      ...
      tanggal:(new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10),
         tanggalRules: [
        v => !!v || 'Tanggal pelunasan belum diisi',
      ],
       fileRules: [
...
//   },
    ...
    async cekNorek (){
      if(this.$gate.isAdmin() || this.$gate.isKredit() || this.$gate.isBisnis() || this.$gate.isPelayanan() || this.$gate.isCs()){
        ...
        //const response = await axios.get('api/pelunasan/ceknama')
          const response = await axios.post('api/pelunasan/ceknorek',formData)
    ...
    initialize() {
      ...
      if (
        ...
        axios.get("api/pelunasan", { params }).then((response) => {
          this.pelunasan = response.data.data;
     ...
     createUser(){
         ...
         this.$Progress.start();
           ...
           // console.log(this.file);
            axios.post('api/pelunasan',formData,config)
              .then((response)=>{
                  $('#addNew').modal('hide');
          ...
          updateUser(){
                ...
                this.form.put('api/pelunasan/'+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                ...
                Swal.fire({
                    ...
                    confirmButtonText: 'Ya, Hapus!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('api/pelunasan/'+id).then(()=>{
                                        Swal.fire(
                                        'Dihapus!',
                                        'Data telah dihapus.',
                                        'success'
